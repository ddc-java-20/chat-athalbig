...
public class MessageService implements AbstractMessageService {
  ...
  private DeferredResult<List<Message>> setUpPolling(Channel channel, Instant since) {
    ...
    futurePolling[0] = scheduler.scheduleWithFixedDelay(pollingTask, POLLING_INTERVAL_MS,
  ...
  private void checkForNewMessages(Channel channel, Instant since, DeferredResult<List<Message>> result,
      ScheduledFuture<?>[] futurePolling) {
    Instant effectiveSince = getEffectiveSince(since);
    if (!messageRepository
        .getLastPostedByChannelAndPostedAfter(channel, effectiveSince, TOP_ONE)
        .isEmpty()) {
      result.setResult(messageRepository.getAllByChannelAndPostedAfterOrderByPostedAsc(channel,
          effectiveSince));
      futurePolling[0].cancel(true);
    }
  }
  ...
  private List<Message> getSinceAtMost(Instant since, Channel channel) {
    Instant effectiveSince = getEffectiveSince(since);
    return messageRepository.getAllByChannelAndPostedAfterOrderByPostedAsc(channel, effectiveSince);
  }
